      ...
      {items.map((step, i) => (
        <div key={i} className="process-step relative">
          <div className="flex items-center gap-3 mb-4">
            <span className="w-10 h-10 rounded-full border border-ivory/30 flex items-center justify-center text-ivory font-mono text-sm font-semibold">
              {step.number}
            </span>
            {i < items.length - 1 && (
              <div className="hidden md:block flex-1 h-px bg-gradient-to-r from-ivory/30 to-transparent" />
            )}
          </div>
